...
def Defensive_Stats():
    ...
    for i in range (13,18):
        df = pd.read_csv("/Users/JonC/Basketball/Player_Csvs/Defense_Tracking/Defense%d.csv" % (i))
    ...
    for i in range (96,100):
        df1 = pd.read_csv("/Users/JonC/Basketball/Player_Csvs/Defensive_General/Defense%d.csv" % (i))
        df1['Year'] = Series ([i+1900 for k in range(len(df1.index))])
        Defense_General = Defense_General.append(df1)

    for j in range (18):
        df2 = pd.read_csv("/Users/JonC/Basketball/Player_Csvs/Defensive_General/Defense%d.csv" % (j))
        df2['Year'] = Series ([j+2000 for k in range(len(df2.index))])
        Defense_General = Defense_General.append(df2)
    Defense_General = Defense_General.rename(columns = {' TEAM_ABBREVIATION':'Team',' PLAYER_NAME': 'Player'})
    Defense_General['Player'] = Defense_General['Player'].astype(str).apply(cleanup)
    #Shooting
    Shooting = DataFrame()
    for i in range (96,100):
        df1 = pd.read_csv("/Users/JonC/Basketball/Player_Csvs/Opponent_Shooting/Shooting%d.csv" % (i))
        df1['Year'] = Series ([i+1900 for k in range(len(df1.index))])
        Shooting = Shooting.append(df1)
    for j in range (18):
        df2 = pd.read_csv("/Users/JonC/Basketball/Player_Csvs/Opponent_Shooting/Shooting%d.csv" % (j))
        df2['Year'] = Series ([j+2000 for k in range(len(df2.index))])
    ...
    Shooting = Shooting.rename(columns = {' TEAM_ABBREVIATION':'Team',' OPP_FGM': "Less than 5 Ft FGM", ' OPP_FGA' : "Less than 5 Ft FGA",
           ...
           ' OPP_FG_PCT.5' : "25-20 Ft FG%"," PLAYER_NAME":"Player"})
    ...
    Shooting = Shooting.drop(['PLAYER_ID',' TEAM_ID',' OPP_FGM.6',' OPP_FGA.6',' OPP_FG_PCT.6',' OPP_FGM.7',
                   ' OPP_FGA.7',' OPP_FG_PCT.7',' OPP_FGM.8',' OPP_FG_PCT.8'],axis=1)

    #Hustle Stats
    Hustle = DataFrame()
    for i in range (15,18):
        df = pd.read_csv("/Users/JonC/Basketball/Player_Csvs/Hustle/Hustle%d.csv" % (i))
        df['Year'] = Series ([2000+i for k in range(len(df.index))])
